...
if(!empty($_POST)) {
    ...
    $en_yakin_sayi = $sayi;
    while($en_yakin_sayi <= $sayi) {
        if(($en_yakin_sayi % 3) == 0)
            break;
        $en_yakin_sayi--;
    }
    echo "Girdiğiniz sayıdan küçük ve o sayıya en yakın üçe bölünen sayı $en_yakin_sayi dır.";
}
?>
</body>
</html>
...
<body>
<form action="online.php" method="post">
    Tablo satır sayısını giriniz :
    <input type="text" name="satir_sayisi" />
    <input type="submit" name="submit" value="Oluştur" />
</form>
<?php
if(!empty($_POST)) {
    $toplam_satir = intval($_POST["satir_sayisi"]);
    echo "<table border='1'>";
    $renk="yellow";
    for($i=1; $i<=$toplam_satir; $i++) {
        echo "<tr bgcolor='$renk'>";
        echo "<td>{$i}_1</td>";
        echo "<td>{$i}_2</td>";
        echo "</tr>";
        $renk=($renk=="green") ? "yellow" : "green";
    }
    echo "</table>";
}
?>
