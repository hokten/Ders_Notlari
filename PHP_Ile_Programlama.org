...
** try-catch Yapısı
try-catch, hata ve istisna tespiti için kullanılan bir yapıdır.
#+BEGIN_SRC php
  try {
...
#+end_example
** include Deyimi
Harici dosyalar ~include~ deyimi ile çalışılan koda eklenebilir. Bu dosyalar, çalıştırılabilir dosyalar ise çalıştırma işlemi de yapılacaktır.
...
~include~ deyiminden sonra gelen ve koda eklenecek olan dosyayı gösteren parametre, bir dosya yolu olmak zorundadır. Bu dosya yolu, PHP'nin ayarlarında yer ala ~include_path~ klasörü içinde aranır. İlgili dosyayı ~include_path~ ayarında belirtilen klasörlerde bulamazsa, ~include~ deyiminin kullanıldığı PHP dosyasının bulunduğu klasöre bakılır.
