...
#+begin_src php
  try {
      $db = new PDO("mysql:host=localhost;dbname=ogrenciler1", "okten1", "QBsfA6Sg73gnfRFg");
      echo "Devam";
  }
  catch (PDOException $e) {
      echo $e->getMessage();
...
#+begin_src php
  try {
      $db = new PDO("mysql:host=localhost;dbname=ogrenciler", "okten", "QBsfA6Sg73gnfRFg");
      $sorgu = "INSERT INTO new_table (numara, ad, soyad, bolum, dogumyili) VALUES(128, 'Zehra', 'FIRAT', 'Harita', 2000)";
...
|    126 | Fatma | MERT  | İnşaat |      2000 |
|    127 | Ayşe  | MERT  | İnşaat |      2000 |
|    128 | Zehra | FIRAT | Harita |      2000 |

***** Veritabanındaki Kayıtı Güncelleme
~new_table~ isimli bir veritabanındaki kayıtları güncellemek için aşağıdaki şekilde bir kod kullanılabilir.
Bu kod numarası 124 olan öğrencinin veya öğrencilerin bölümünü Elektronik olarak değiştirmektedir. Kullandığımız tabloda, numarası 124 olan tek bir kayıt olduğundan dolayı, bu işlemden etkilenen kayıt sayısı 1 olacaktır. 
#+begin_src php
  try {
      $db = new PDO("mysql:host=localhost;dbname=ogrenciler", "okten", "QBsfA6Sg73gnfRFg");
...
#+begin_src php
  try {
      $db = new PDO("mysql:host=localhost;dbname=ogrenciler", "okten", "QBsfA6Sg73gnfRFg");
      $sorgu = "UPDATE new_table SET bolum='Makine' WHERE soyad='MERT'"; 
      $etkilenen = $db->exec($sorgu);
      echo "İşlemden etkilenen kayıt satırı sayısı : $etkilenen";
  }
  catch (PDOException $e) {
...
|--------+-------+-------+--------+-----------|
|    125 | Ali   | VELİ  | İnşaat |      2000 |
|    126 | Fatma | MERT  | Makine |      2000 |
|    127 | Ayşe  | MERT  | Makine |      2000 |
|    128 | Zehra | FIRAT | Harita |      2000 |
...
#+begin_src php
  try {
      $db = new PDO("mysql:host=localhost;dbname=ogrenciler", "okten", "QBsfA6Sg73gnfRFg");
      $sorgu = "SELECT * FROM new_table WHERE bolum='Makine'";
...
#+begin_src php
  try {
      ...
      echo "Toplam $durum adet kayıt listelenecektir.";
...
#+RESULTS:
: Toplam 1 adet kayıt listelenecektir.
