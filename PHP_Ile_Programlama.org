...
Bazı PHP fonksiyonlarının görevlerini icra edebilmesi için parametre olarak değişkenden daha fazlasına ihtiyaç duyarlar. Böyle fonksiyonlar, bu durumun üstrsinden gelmek için başka bir fonksiyonu parametre olarak kullanabilirler. Örneğin, ~usort(array &$array, callable $callback) : bool~ fonksiyonu bir dizinin elemanlarını sıralar. İkinci parametresi ~$callback~ ~callable~ türündendir. ~callable~ türü bu değişkenin bir fonksiyon olması gerektiğini ifade eder. Açıktır ki bu fonksiyon, dizini elemanlarının nasıl sıralacağını belirleyen fonksiyondur.
