...
#+begin_src php
  $array = array("really long string here, boy", "this", "middling
  length",
                 "larger");
  $sortOption = "random";
...
** PDO Kullanımı
PDO, veritabanı işlemeri için kullanılan bir PHP eklentisidir. Bir çok veritabanı türünü destekler. Programınızda PDO kullanabilmek için, PHP konfigürasyonunuzda PDO eklentisini ve PDO'nun kuullanacağınız veritabanı türüne ait eklentisini aktif hale getirmek gerekir. Örneğin PDO mysql veritabanı için kullanılacaksa php.ini dosyası
...
- Veritabanı sunucsuna ait bir kullanıcı ismi ve bu kullanıcının şifresi.
...
Veritabanı üzerinde en çok yapılan işlemler: sorgu, düzenleme, ekleme ve silmedir.
Veritabanı sorgularını aşğıdaki şekilde gruplandırmak mümkündür:

1. Veri Döndürmeyen Sorgular
   INSERT, UPDATE ve DELETE gibi SQL sorgularının veritabanı tablosundan veri döndürmesi beklenmez. Bu durumda ~PDO~ sınıfının ~exec()~ fonksiyonu kullanılabilir. ~exec~ fonksiyonu yalnızca veritabanı sorgusundan etkilenen kayıtların sayısını döndürür.
2. Veri Döndürmesi Beklenen Sorgular.
Veritabanı tablosundan kayıt veya kayıtlar döndürmesi beklenen sorgular için ~query~ fonksiyonu kullanılır.

Örneklerde kullanacağımız veritabanının yapısı:

#+PROPERTY: header-args:sql :engine mysql :dbhost localhost :dbuser root :dbpassword root :database ajax_crud
#+begin_src sql
...
|  3 | 2017_02_28_042012_create_tasks_table           |     1 |

| numara | ad   | soyad  | vize | final | durum |
|    123 | Ali  | ŞAHİN  |   34 |    56 |       |
|    124 | Veli | ŞİMŞEK |   34 |    76 |       |
|    125 | Akif | TAŞ    |   67 |    76 |       |

**** Veritabanı Tablosuna Veri Ekleme
~tablo~ isimli bir veritabanına kayıt eklemek için aşağıdaki şekilde bir kod kullanılabilir.
#+begin_src php
  $sorgu = "";
  $etkilenen = $dbh->exec($sorgu);
  echo "İşlemden etkilenen kayıt stırı sayısı : $affected";
#+end_src
